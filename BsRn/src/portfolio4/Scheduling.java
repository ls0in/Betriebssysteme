...
public class Scheduling {
	...
	public void FCFS(ArrayList<Prozess>sizes){									 			
		...
		for (int k = 1; k < sizes.size(); k++) {
			if (ankunftsZeit[k]<=finishedTimes[k-1]) {
				...
				finishedTimes[k]=ankunftsZeit[k]+laufZeit[k];//falls Ankunftszeit eines Prozesses nach der Laufzeit des Vorgängers liegt.
			}
		}	
		
		//KONSOLENAUSGABE
		
		//Ausgabe Zeitintervall (00	01	02	03...)	
		int length=finishedTimes[finishedTimes.length-1];
		for (int i = 0; i <length ; i++) {
			if (i<10) {
				System.out.print("\t"+"0"+i);
	...
	public void SRTF(ArrayList<Prozess>sizes){
		...
		for (int i = 0; i < tempProzess.size(); i++) {
			for (int j = 0; j < tempProzess.get(i).getLaufZeit(); j++) {
				counter++;
				for (int k = 0; k < tempProzess.size(); k++) {
					if (counter==tempProzess.get(i+1).getAnkuftsZeit()) {
